...
export class HasuraService {
	...
	filterJobs(jobs, filters, search) {
		...
		const handleAnnualIncome = (
			...
			else if (parseFloat(cleanFilterValue) <= 100000) {
				// Assuming monthly income won't exceed 1L
		...
		const evaluateCondition = (tagValueJson, filterKey, filterValue) => {
			try {
				...
				const cleanConditionValues = conditionValues.map((v) =>
				...
				switch (condition) {
					...
					case '=':
						return (
							cleanConditionValues[0] === cleanFilterValue ||
							cleanConditionValues[0].replace(/[^a-zA-Z0-9]/g, '') ===
								cleanFilterValue.replace(/[^a-zA-Z0-9]/g, '')
						);
					...
					case '>=': {
						...
						return (
							!isNaN(filterNumGt) &&
							!isNaN(conditionNumGt) &&
							filterNumGt >= conditionNumGt
						);
					}
					...
					case '<': {
						...
						return (
							!isNaN(filterNumLt) &&
							!isNaN(conditionNumLt) &&
							filterNumLt < conditionNumLt
						);
					}
					...
					case '>': {
						...
						return (
							!isNaN(filterNumGt2) &&
							!isNaN(conditionNumGt2) &&
							filterNumGt2 > conditionNumGt2
						);
					}

					default: {
						const valueIncludes =
							cleanConditionValues[0].includes(cleanFilterValue);
						const filterIncludes = cleanFilterValue.includes(
							cleanConditionValues[0],
						);
	...
	async insertCacheData(arrayOfObjects: any[], bpps: string[]) {
		...
		if (arrayOfObjects.length > 0 && bpps.length > 0) {
			try {
		...
		this.logger.log(
			`Cache refresh completed: inserted ${insertApiRes.length} records`,
		);
		return insertApiRes;
	}
	...
	async deleteJobs() {
		...
		try {
			...
			this.logger.log(
				`Deleted ${result.data.delete_ubi_network_cache.affected_rows} records from cache`,
			);
			return result;
		} catch (error) {
