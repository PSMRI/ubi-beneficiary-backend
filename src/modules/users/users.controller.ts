...
export class UserController {
	...
	@UseGuards(AuthGuard)
	@ApiOperation({
		summary: 'Fetch Verifiable Credential JSON from a QR code URL',
	})
	@ApiResponse({ status: 200, description: 'VC JSON fetched successfully' })
	@ApiResponse({
		status: 400,
		description: 'Invalid URL or unable to fetch VC JSON',
	})
	...
	@ApiBasicAuth('access-token')
	@ApiOperation({
		summary: 'Update application status for the authenticated user',
	})
	@ApiResponse({
		status: 200,
	...
	@ApiResponse({ status: 401, description: 'Unauthorized' })
	@ApiResponse({ status: 500, description: 'Internal server error' })
	async updateApplicationStatus(@Req() req: Request) {
		try {
			return await this.userService.processApplicationStatusUpdatesForAuthenticatedUser(
				req,
			);
		} catch (error) {
