...
export class ApplicationStatusUpdate {
	...
	async updateApplicationStatusCron() {
		try {
			Logger.log(
				'Application Status Update CRON started at ' + new Date().toISOString(),
			);
			...
			Logger.log(
				`Application Status Update CRON completed successfully. ${(result.data as any)?.processedCount || 0} applications processed.`,
			);
		} catch (error) {
